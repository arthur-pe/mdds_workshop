...
def evaluate(key, vector_fields, controls, stepsize_controller, saveat, solver, t0, t1, intrinsic_noise, extrinsic_noise):
    ...
    if intrinsic_noise != 0: terms.append(ControlTerm(lambda t, x, args: vector_fields.f(t, x, args) * jnp.linalg.norm(controls(t)) * intrinsic_noise,
                                                      VirtualBrownianTree(t0=t0 - 10 ** -5, t1=t1 + 10 ** -5, tol=1e-2,
                                                                          shape=(vector_fields.intrinsic_dim,),
                                                                          key=key)))
